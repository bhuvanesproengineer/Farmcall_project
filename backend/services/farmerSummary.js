export async function getFarmerSummary(req, weatherSummary) {
  try {
    ...
    const village = params.village || 'your village';
    const examples = {
      English: `Example:
...
1. Greet the farmer by name (e.g., "Good morning, ${farmerName}").
...
`;
    ...
    console.error("[getFarmerSummary Error]:", error.message || error);
    console.warn(`[getFarmerSummary] Falling back to template-based summary for language: ${language}`);
    ...
    const fallbacks = {
      Telugu: `శుభోదయం ${farmerName} గారు. ${village} లో నేటి వాతావరణ సమాచారం: ${weatherSummary}. దయచేసి వ్యవసాయానికి తగిన జాగ్రత్తలు తీసుకోండి. ధన్యవాదాలు.`,
      Tamil: `வணக்கம் ${farmerName} அவர்களே. ${village} பகுதியில் இன்றைய வானிலை: ${weatherSummary}. தேவையான முன்னெச்சரிக்கை நடவடிக்கைகளை எடுக்கவும். நன்றி.`,
      Hindi: `नमस्ते ${farmerName} जी। ${village} में आज का मौसम: ${weatherSummary}। कृपया आवश्यक कृषि सावधानियां बरतें। धन्यवाद।`,
      Kannada: `ಶುಭೋದಯ ${farmerName} ಅವರೆ. ${village} ಯಲ್ಲಿ ಇಂದಿನ ಹವಾಮಾನ: ${weatherSummary}. ದಯವಿಟ್ಟು ಅಗತ್ಯ ಕೃಷಿ ಮುನ್ನೆಚ್ಚರಿಕೆಗಳನ್ನು ತೆಗೆದುಕೊಳ್ಳಿ. ಧನ್ಯವಾದಗಳು.`,
      Malayalam: `സുപ്രഭാതം ${farmerName}. ${village} പ്രദേശത്തെ ഇന്നത്തെ കാലാവസ്ഥ: ${weatherSummary}. ആവശ്യമായ കാർഷിക മുൻകരുതലുകൾ എടുക്കുക. നന്ദി.`,
      English: `Good morning ${farmerName}. Here is the weather advisory for ${village}: ${weatherSummary}. Please take necessary farming precautions. Thank you.`
    };
